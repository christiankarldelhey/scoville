...
const PlayerRooms = ({ rooms }: PlayerRoomsProps) => {
  ...
  const renderRoomSlot = (room: RoomCard | undefined) => {
    ...
    return (
      <div className="border border-gray-400 flex items-center justify-center bg-gray-100 relative rounded-md">
          ...
          <>
            ...
            {room.guest && (
              <>
                ...
                <div 
                  className={`absolute bottom-0 left-0 w-[30px] h-[30px] ${getSuitColor(room.guest.suit)} rounded-tr-full flex items-center justify-center`}
                >
                  <img src={getProductSymbol(room.guest.product)} alt={room.guest.product} className="h-4 w-4" />
                </div>
                
                {/* Cuarto de círculo superior derecho - Nights */}
                <div 
                  className={`absolute top-0 right-0 w-[30px] h-[30px] ${getSuitColor(room.guest.suit)} rounded-bl-full flex items-end justify-end p-0.5`}
                >
